...
(function() {
  ...
  (function($) {
    ...
    changeTooltipColorTo = function(color) {
      ...
      jQuery('.tooltip-inner').css('background-color', color);
      jQuery('.tooltip.top .tooltip-arrow').css('border-top-color', color);
      jQuery('.tooltip.right .tooltip-arrow').css('border-right-color', color);
      jQuery('.tooltip.left .tooltip-arrow').css('border-left-color', color);
      jQuery('.tooltip.bottom .tooltip-arrow').css('border-bottom-color', color);
      return null;
    };
    ...
    initIsotope = function(args) {
      ...
      if (jQuery().isotope) {
        ...
        if (args.hasFilters) {
          jQuery(args.filters).on('click', function(e) {
            e.preventDefault();
            jQuery(args.filters).removeClass('active');
    ...
    initMenuAnimation = function() {
      if ($(window).width() >= 768) {
        jQuery('.dropdown').hover(function() {
          ...
          jQuery(this).find('.dropdown-menu').first().stop(true, true).delay(250).fadeOut().slideUp('slow');
          return null;
        });
      }
      return null;
